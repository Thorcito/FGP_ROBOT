...
bool SimpleEffortControl::init()
{
    ...
    {
        s.str("");
        s<<"SimpleEffortControl init(): Wrong number of i_gains --"<<p.size()<<"--";
        m_error=true;
        m_errorString=s.str().c_str();
        return false;
    }
...
VectorDOFd SimpleEffortControl::update(const RobotTime& time, const JointState &current)
{
    if(!m_startFlag)
    {
        m_qStart=current.q;
...
//    Vector6d G=Gc(current);
    ...
    tau=-m_Kd*Sq + Yr*Th + B*current.qp;
    ...
    for(int i=0;i<STD_DOF;i++)
    {
        ...
        {
            std::stringstream s;
            s<<"SimpleEffortControl update(): The control output is nan/inf: "<<tau.transpose();
            m_error=true;
            m_errorString=s.str().c_str();
...
VectorDOFd SimpleEffortControl::Gc(const JointState& js)
{
    VectorDOFd G;
