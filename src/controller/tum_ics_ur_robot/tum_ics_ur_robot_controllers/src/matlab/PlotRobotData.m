function PlotRobotData( name, level, fig)
...
if(level>0)
    ...
    for i=0:5
        subplot(3,2,i+1);
        plot(t,x(:,joint_idx+(i)),'b -',t,x(:,jointf_idx+i),'r -')
...
if(level>0)
    ...
    for i=0:5
        subplot(3,2,i+1);
        plot(t,x(:,jointp_idx+(i)),'b -',t,x(:,jointpf_idx+i),'r -')
        ns=sprintf('joint V %d',i+1);
        title(ns)
        xlabel('time [s]')
        ylabel('deg/s')
        grid on
...
if(level>0)
    ...
    for i=0:5
        subplot(3,2,i+1);
        plot(t,x(:,jointpp_idx+(i)),'b -',t,x(:,jointppf_idx+i),'r -')
        ns=sprintf('joint A %d',i+1);
        title(ns)
        xlabel('time [s]')
        ylabel('deg^2/s')
        grid on
