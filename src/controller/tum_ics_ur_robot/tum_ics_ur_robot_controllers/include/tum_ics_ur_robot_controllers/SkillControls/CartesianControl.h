#ifndef UR_ROBOT_LLI_CARTESIAN_CONTROL_H
#define UR_ROBOT_LLI_CARTESIAN_CONTROL_H
...
class CartesianControl: public ControlEffort
{
    // Member variables
private:
    ...
    // Member Methods
public:
    CartesianControl(Robot::KinematicModel *kModel,
                  Robot::DynamicModel *dModel=NULL,
                  double weight=1.0,
                  const QString& name="CartesianCtrl");
    ~CartesianControl();

    void setQInit(const JointState& qinit);
    void setQHome(const JointState& qhome);
    void setQPark(const JointState& qpark);
    void getXdfromTopic(const geometry_msgs::Pose::ConstPtr &msg);
    void enableXdTopic();
    void disableQdTopic();
    const bool isXdTopicEnabled() const;
...
#endif // UR_ROBOT_LLI_CARTESIAN_CONTROL_H
