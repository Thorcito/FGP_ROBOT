#!/usr/bin/env python3
"""
BallKFPredictor_planeIntercpet (plane-interception criterion – prediction-only)
...
- This node purposely does NOT compute a plane intercept; it only produces
  filtered states and ballistic predictions. The downstream "bridge_plane"
...
class BallKFPredictor_planeIntercept:
    def __init__(self):
        # ---------- params ----------
        ...
        rospy.loginfo("BallKFPredictor_planeIntercept: frame=%s, g_cam=%s, lookahead=%.2fs, horizon=%.2fs@%.0fHz",
...
if __name__ == "__main__":
    rospy.init_node("ball_kf_predictor_planeIntercept")
    BallKFPredictor_planeIntercept()
    rospy.spin()
